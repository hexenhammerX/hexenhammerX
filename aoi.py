...
async def cmd_start(message: types.Message):
    await message.answer('VVEDITE INN')
    global searching_by_INN
    searching_by_INN = True
...
async def filter_options(message: types.Message, state: FSMContext):
    ...
    message_info = ('*Наименование:* ' + res1_variable_path['НаимСокрЮЛ']
                    + '*\nОБЩАЯ ИНФОРМАЦИЯ*'
                    + '\n'
                    + '*\nИНН:* ' + res1_variable_path['ИНН']
                    + '*\nВозраст:* ' + str(company_age) + 'регистрация: ' + res2_variable_path['ДатаРег']
                    #+ '*\nОГРН:* ' + OGRN_or_OGRNIP
                    + '*\nАдрес:* ' + res1_variable_path['АдресПолн']
                    #+ '*\nДата Регистрации:* ' + json_res2['items'][0]['ЮЛ']['ДатаРег']
                    + '*\nУставный капитал:* ' + res2_variable_path['Капитал']['СумКап']
